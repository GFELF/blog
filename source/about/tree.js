...
var width = mainDiv.offsetWidth *3/4;
...
for (var i = 0; i < n; i++) {
    ...
    // 根据序号i算出初始x坐标
    branch.x = width *2/3 - initialRadius + i * 2 * initialRadius / n;
    branch.radius = initialRadius;

    // 将新的branch加入集合中去
    branches.add(branch);
}
var str = '光秃秃的树，像孤零零的我'
var list = str.split('')
var p = document.querySelector('p')
p.innerText = ''
//console.log(branches)
var intervalTime = 250 * 20 / list.length /1.5
var count = 0
var textInterval = setInterval(function() {
    if (list.length > 0) {
        if (count % 2 === 0){
            p.innerText = p.innerText.replace('_', '') + list.shift()
        } else {
	...
	p.onclick = function() {location.replace('http://blog.gefei.fun');}
    }	
}, intervalTime);
